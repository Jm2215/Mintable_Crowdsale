...
module.exports = async function(deployer, network, accounts) {
  deployer.deploy(TestToken, "Test Token", "TTS", 18, 1000e18, 0xEFF83f0b41a0aC137e14287364C1216B482a2aaF).then(async () => {
  ...
  //       uint256 _cap
    const rate = 100; // 1 eth = 100 TTS tokens
    const wallet = accounts[0];
    const timeNow = Math.floor(Date.now() / 1000);
    const openingTime = timeNow  + duration.seconds(30);
    const closingTime = timeNow  + duration.hours(2);
